...
static u32 MSYSTICK_PdUS(u32 copy_u16TimePeriod)
{
	u32 Local_u32TicksNumber,AHB_Clock;
	f32 Local_f32TickPeriod;
	u8 AHB_Prescaler;
...
void MSYSTICK_voidDelayms(u32 Copy_u32TimeINms)
{
    ...
    u32 ticks_per_ms = SYSTEM_CLOCK / 1000;
    u32 max_ticks = 0xFFFFFF; // 24-bit maximum

    while (Copy_u32TimeINms > 0)
    {
        u32 delay_ticks;
        if(Copy_u32TimeINms > max_ticks / ticks_per_ms){
        	delay_ticks = max_ticks;
        }
        else {
        	delay_ticks = Copy_u32TimeINms * ticks_per_ms;
        }

        SYSTICK->STK_LOAD = delay_ticks - 1; // Load ticks
        SYSTICK->STK_VAL = 0; // Clear current value
        SET_BIT(SYSTICK->STK_CTRL, STK_CTRL_ENABLE); // Start timer

        while (GET_BIT(SYSTICK->STK_CTRL, STK_CTRL_COUNTFLAG) == 0); // Wait for timeout

        CLR_BIT(SYSTICK->STK_CTRL, STK_CTRL_ENABLE); // Stop timer
        Copy_u32TimeINms -= (delay_ticks / ticks_per_ms);
    }
